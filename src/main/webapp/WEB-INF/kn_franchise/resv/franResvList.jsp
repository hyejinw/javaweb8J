		    ...
		    columns: [
  				...
  				{data: "pickupDate"},
  				{
  					...
  					render: function(data) {
  						if(data == 'NO') {
	  						return '<button class="btn btn-primary btn-sm" data-toggle="modal" data-target="#pickupClick">'+data+'</button>'
  						}
  						else {
...
/* 		    
		...
		function resvOk() {
			let pwd = resvForm.pwd.value;
			let idx = resvForm.idx.value;
			let pickupDate = resvForm.pickupDate.value;
			...
			if(pwd != '확인') {
				alert('확인 메시지를 잘못 입력하셨습니다.');
				return false;				
			}
			
 			if(pickupDate > todayDate) {
				alert('픽업 예약일 전입니다. 픽업 승인을 하실 수 없습니다.');
				return false;				
			}			
...
<body>
 	<div id="c">
		...
		<div style="text-align:center"  class="mb-5"><span class="text-center" style="font-size:15px; text-align:center; font-weight:300; color:blue">픽업 유무를 확인해주세요</span></div>
    <table id="resvTable" class="table table-hover table-bordered">
	 	...
	 	<!-- The Modal -->
  <div class="modal fade" id="pickupClick">
    <div class="modal-dialog modal-dialog-centered">
      <div class="modal-content">
      
        <!-- Modal Header -->
        <div class="modal-header">
          <h4 class="modal-title text-center">픽업 확인창</h4>
        ...
        <div class="modal-body">
        	<form name="resvForm">
		      	<div class="row mb-4">
		      		<div class="col-sm text-danger">해당 예약의 픽업을 승인하시려면 '확인'을 입력해주세요.</div>
		      	</div>
		      	<div class="row mb-2">
		      		<div class="col-sm">메시지: <input type="text" name="pwd" id="pwd" class="form-control"/></div>
        ...
        <div class="modal-footer">
          <button type="button" class="btn btn-secondary" onclick="javascript:resvOk()">픽업 확인</button>
