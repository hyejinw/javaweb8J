	...
	<div style="padding:100px 200px 200px; background-color:rgba(253, 250, 247, 0.8)">
	  ...
	  <div class="text-center">
	  	<ul class="pagination justify-content-center">
		    <c:if test="${pag > 1}"><li class="page-item"><a class="page-link text-secondary" href="${ctp}/ResvMenu.kn_re?pageSize=${pageSize}&pag=1&part=${part}">◁◁</a></li></c:if>
		    <c:if test="${curBlock > 0}"><li class="page-item"><a class="page-link text-secondary" href="${ctp}/ResvMenu.kn_re?pageSize=${pageSize}&pag=${(curBlock-1)*blockSize + 1}&part=${part}">◀</a></li></c:if>
		    <c:forEach var="i" begin="${curBlock*blockSize + 1}" end="${curBlock*blockSize + blockSize}" varStatus="st">
		      <c:if test="${i <= totPage && i == pag}"><li class="page-item active"><a class="page-link text-white bg-secondary border-secondary" href="${ctp}/ResvMenu.kn_re?pageSize=${pageSize}&pag=${i}&part=${part}">${i}</a></li></c:if>
		      <c:if test="${i <= totPage && i != pag}"><li class="page-item"><a class="page-link text-secondary" href="${ctp}/ResvMenu.kn_re?pageSize=${pageSize}&pag=${i}&part=${part}">${i}</a></li></c:if>
		    </c:forEach>
		    <c:if test="${curBlock < lastBlock}"><li class="page-item"><a class="page-link text-secondary" href="${ctp}/ResvMenu.kn_re?pageSize=${pageSize}&pag=${(curBlock+1)*blockSize + 1}&part=${part}">▶</a></li></c:if>
		    <c:if test="${pag < totPage}"><li class="page-item"><a class="page-link text-secondary" href="${ctp}/ResvMenu.kn_re?pageSize=${pageSize}&pag=${totPage}&part=${part}">▷▷</a></li></c:if>
	 		</ul>
	  </div>
		</div>
		</div>
